...
function brand(ctx:CanvasRenderingContext2D,left:HTMLImageElement,right:HTMLImageElement,blue=false){if(blue)ctx.drawImage(left,0,0,327,90,50,955,335,93);else ctx.drawImage(left,50,955,335,93);ctx.drawImage(right,847,960,190,86);}
...
export async function renderPost(canvas: HTMLCanvasElement, state: PostState) {
  ...
  const { templateId, headline, background, wave, headlineColor, logoColor, photos } = state;
    ...
    const [leftLogo,rightLogo,...loaded]=await Promise.all([loadImage(blueLogos?'/assets/fao-blue-two-side.svg':'/assets/fao-un-white-trim.png'),loadImage(blueLogos?'/assets/fao-right-blue-trim.png':'/assets/fao-right-white-trim.png'),...photos.map(p=>p?loadImage(p.src):Promise.resolve(null))]);
