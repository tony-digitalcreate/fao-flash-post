...
export async function renderPost(canvas: HTMLCanvasElement, state: PostState) {
  ...
  const { templateId, headline, background, wave, headlineColor, logoColor, photos } = state;
    ...
    const [leftLogo,rightLogo,...loaded]=await Promise.all([loadImage(blueLogos?'/assets/fao-un-blue-trim.png':'/assets/fao-un-white-trim.png'),loadImage(blueLogos?'/assets/fao-right-blue-trim.png':'/assets/fao-right-white-trim.png'),...photos.map(p=>p?loadImage(p.src):Promise.resolve(null))]);
